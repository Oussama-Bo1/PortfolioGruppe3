...
public class Main {
	...
	JComboBox<String> cbMultipleCityList;
	...
	private void initialize() {
		...
		cbMultipleCityList = new JComboBox<String>();
		cbMultipleCityList.setEditable(false);
		cbMultipleCityList.setBounds(10, 159, 311, 22);
		cbMultipleCityList.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String city = (String) cbMultipleCityList.getSelectedItem();
				int plz = 0;
				for(City c : cities) {
				...
				txtZipcode.setText(""+plz);
			}
		});
		frame.getContentPane().add(cbMultipleCityList);
		...
		btnLookForZipCode.addActionListener(new ActionListener() { 
			public void actionPerformed(ActionEvent e) {
				...
				if(cities.length >= 1 && !(txtZipcode.getText() == " " || txtZipcode.getText().isEmpty())) {
					cbMultipleCityList.removeAllItems();
					for(int i = 0; i < cities.length; i++) {
						...
						cbMultipleCityList.addItem(cities[i].getName());  
					}
					frame.getContentPane().add(cbMultipleCityList);
				} else {
					lblInfo.setText("Keine PLZ für die Stadt gefunden!");
					txtCity.setText("");
					txtZipcode.setText("");
				}
			}
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if(cities.length > 1) {
						...
						lblInfo.setText("Keine Stadt für die Postleitzahl gefunden!");
					}
